...
### [6. 자바스크립트 엔진에서 비동기 처리는 어떻게 이루어지는가?](#6-자바스크립트-엔진에서-비동기-처리는-어떻게-이루어지는가-1)  
...
---
## 6. 자바스크립트 엔진에서 비동기 처리는 어떻게 이루어지는가?  

- 자바스크립트에서 비동기 처리는 이벤트 루프를 통해 이루어 집니다. 이벤트 루프는 3가지 영역으로 나누어 볼 수 있습니다.
- 콜 스택 : 현재 실행 중인 함수의 정보를 담는 공간입니다. 자바스크립트는 하나의 콜 스택을 사용하기 때문에, 실행되는 모든 함수들은 여기에 담깁니다. <br />
stack이므로 블럭이 쌓이는 형태로 생각할 수 있습니다.
- 콜백 큐 : 콜백 함수를 저장하는 공간입니다. (setTimeOut에서 일정 시간이 지난 뒤 실행되는 함수를 예로 들 수 있습니다.) 여기에 쌓인 콜백 함수들은
콜 스택이 빌 때마다 순서대로 콜 스택에 쌓여 실행됩니다.
- 백그라운드 : 비동기 처리가 이루어지는 장소입니다. 타이머, 네트워크 요청 등이 수행됩니다. 비동기 처리가 완료된 뒤, 콜백 함수가 생성된다면 콜백 큐에 저장됩니다.

<br />

### 6-1. 비동기 작업과 콜백 함수는 차이점은 무엇인가?

- 비동기 작업은 작업 그 자체를 의미 합니다. setTimeOut으로 3초뒤 test라는 함수를 실행한다고 가정해보겠습니다. 여기서 비동기 작업은 3초 기다리기(setTimeOut),
콜백 함수는 test 함수를 의미합니다. 하지만 최근에는 콜백이 아닌 promise와 async/await을 사용하여 콜백 함수가 존재하지 않는 경우도 많습니다.

<br />

### 6-2. 마이크로 태스크 큐와 태스크 큐의 차이점은 무엇인가?

- 콜백 큐는 마이크로 태스크 큐와 태스크 큐 2가지로 구분됩니다. 각 영역은 중요도에서 차이가 있습니다.
- 마이크로 태스크 큐는 우선도가 높습니다. promise, async/await 키워드로 실행되는 비동기 작업의 콜백함수는 여기에 저장됩니다. 콜스택이 비어 있을시 <br />
마이크로 태스크 큐에 저장된 콜백 함수가 실행됩니다. (setTImeOut등은 일정 시간을 조건으로 하지만, promsie, async/await 등은 실행 완료 즉시 응답을 주는 것이 <br />
좋기 때문에 우선도가 높다고 할 수 있습니다.)
- 태스크 큐는 우선도가 낮습니다. setTimeOut 등으로 실행되는 비동기 작업의 콜백함수가 여기에 저장됩니다. 
...
- 콜백 함수는 다른 함수의 인자로 전달되어 실행되는 함수를 의미합니다. <br />
다른 함수에 인자로 추가되어, 실행될 때 모두 콜백 함수로 사용된다고 볼 수 있습니다.

<br />

### 7-1. "callback hell"은 무엇인가?

- 비동기 작업을 위해 콜백 함수를 중첩하는 과정에서 코드의 가독성이 떨어지고, 유지 보수가 어려워 지는 문제를 말합니다.
```
A(function(resultA) {
  B(resultA, function(resultB) {
    C(resultB, function(resultC) {
      D(resultC, function(resultD) {
        // resultD를 이용해 다음 작업 수행
      });
    });
  });
});
```
- 이러한 무한 중첩을 피하기 위해 promise, async/await 키워드를 사용하는 것이 좋습니다.
...
- promise는 자바스크립트에서 비동기 처리를 위한 객체입니다. 
- promise는 3가지 상태가 있습니다. 대기(pending) : 결과 미지정 상태, 이행(fulfilled): 성공, 거부(rejected): 실패
- Promise가 이행 상태일 때는 then()메서드, 거부 상태일 때는 catch()메서드를 이용해 결과 값을 처리할 수 있습니다.

<br />

### 8-1. promiseAll은 무엇인가?

- 여러 개의 프로미스 객체를 인자로 받아 모든 프로미스 객체가 완료될 때까지 기다린 다음, 모든 프로미스 객체가 완료되면,
각 프로미스 객체에서 반환된 결과값들을 배열로 반환하는 메서드입니다.

- 여러개의 비동기 작업들이 모두 완료된 뒤에 작업을 수행해야 할 때 사용할 수 있습니다.

- 예를들어, "A, B, C 회사에서 연락온 시간 퇴근할 때 보고서에 적어서 제출해" 같은 예시를 들 수 있습니다.
...
- promise를 기반으로 하지만, promise를 더 쉽게 사용할 수 있게 해줍니다.
- 비동기 작업을 동기식 코드처럼 작성할 수 있습니다.
- 함수에 async 키워드를 사용하고 해당 함수 내부에서 await 키워드를 사용하면 promise가 resolve 될 때까지 대기 합니다.

<br />

### 9-1. async/await에서 에러 처리는 어떻게 하는가?

- 함수 내부에서 try-catch문을 사용하여 에러 처리를 할 수 있습니다.
- try 내부에서 코드를 진행하다가 에러가 발생할 시 catch 내부의 코드를 실행하게 됩니다.
